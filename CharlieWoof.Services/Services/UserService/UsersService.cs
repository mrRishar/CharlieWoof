...
namespace CharlieWoof.Services.Services.UserService
{
    public class UsersService : StatableService<User, User, Guid, Guid>, IUsersService
    {
        ...
        public new User Add(User user,  Guid updatedByUserId)
        {
            var newPasswordHash = Sha256GeneratorAdapter.Generate(user.PasswordHash);

            user.PasswordHash = newPasswordHash;
            return base.Add(user, updatedByUserId);
        }
        ...
        {
            return this.Get(x => x.Email.Equals(email, StringComparison.InvariantCultureIgnoreCase));
        }
    }
}